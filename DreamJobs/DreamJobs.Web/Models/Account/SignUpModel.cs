...
namespace DreamJobs.Web.Models.Account
{
    public class SignUpModel : BaseModel
    {
        ...
        public SignUpModel(ISignInService<ApplicationUser> signInService,
        ...
        public async Task<(IdentityResult, ApplicationUser)> CreateUserAsync()
        {
            ...
            if (result.Succeeded)
            {
                try
                {
                    await _userService.AddClaimAsync(user, new Claim(MembershipClaims.MemberClaimType, MembershipClaims.MemberClaimValue));
